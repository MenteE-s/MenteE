...
const API_ROOT = getNormalizedApiBase();
...
const resolveUploadUrl = (relativePath) => {
...
const Card = ({ children, className = "", shadowClass = "shadow" }) => (
  <div
    className={`bg-white rounded-2xl border border-slate-200 ${shadowClass} p-6 ${className}`}
  >
    {children}
  </div>
);
...
export default function Profile() {
    ...
    const loadProfileData = async () => {
      try {
        ...
        const [
          ...
          authFetch("/profile/sections"),
          authFetch("/profile/experiences"),
          authFetch("/profile/educations"),
          authFetch("/profile/skills"),
          authFetch("/profile/projects"),
          authFetch("/profile/publications"),
          authFetch("/profile/awards"),
          authFetch("/profile/certifications"),
          authFetch("/profile/languages"),
          authFetch("/profile/volunteer-experiences"),
          authFetch("/profile/references"),
          authFetch("/profile/hobby-interests"),
          authFetch("/profile/professional-memberships"),
          authFetch("/profile/patents"),
          authFetch("/profile/course-trainings"),
          authFetch("/profile/social-media-links"),
          authFetch("/profile/key-achievements"),
          authFetch("/profile/conferences"),
          authFetch("/profile/speaking-engagements"),
          authFetch("/profile/licenses"),
        ]);
  ...
  const confirmDelete = async () => {
    ...
    try {
      let endpoint = "";
      if (type === "experiences") endpoint = `/profile/experiences/${item.id}`;
      else if (type === "educations")
        endpoint = `/profile/educations/${item.id}`;
      else if (type === "skills") endpoint = `/profile/skills/${item.id}`;
      else if (type === "projects") endpoint = `/profile/projects/${item.id}`;
      else if (type === "publications")
        endpoint = `/profile/publications/${item.id}`;
      else if (type === "awards") endpoint = `/profile/awards/${item.id}`;
      else if (type === "certifications")
        endpoint = `/profile/certifications/${item.id}`;
      else if (type === "languages") endpoint = `/profile/languages/${item.id}`;
      else if (type === "volunteerExperiences")
        endpoint = `/profile/volunteer-experiences/${item.id}`;
      else if (type === "references")
        endpoint = `/profile/references/${item.id}`;
      else if (type === "hobbyInterests")
        endpoint = `/profile/hobby-interests/${item.id}`;
      else if (type === "professionalMemberships")
        endpoint = `/profile/professional-memberships/${item.id}`;
      else if (type === "patents") endpoint = `/profile/patents/${item.id}`;
      else if (type === "courseTrainings")
        endpoint = `/profile/course-trainings/${item.id}`;
      else if (type === "socialMediaLinks")
        endpoint = `/profile/social-media-links/${item.id}`;
      else if (type === "keyAchievements")
        endpoint = `/profile/key-achievements/${item.id}`;
      else if (type === "conferences")
        endpoint = `/profile/conferences/${item.id}`;
      else if (type === "speakingEngagements")
        endpoint = `/profile/speaking-engagements/${item.id}`;
      else if (type === "licenses") endpoint = `/profile/licenses/${item.id}`;
  ...
  const saveAbout = async (aboutData) => {
    ...
    try {
      const response = await authFetch("/profile/sections", {
        method: "POST",
        body: JSON.stringify({
          section_type: "about",
  ...
  const savePersonalInfo = async (personalData) => {
    ...
    try {
      const response = await authFetch("/cvai/profile", {
        method: "PUT",
  ...
  const handleProfilePictureUpload = async (event) => {
    ...
    try {
      ...
      const response = await authFetch("/profile/upload-profile-picture", {
        method: "POST",
        body: formData,
      });
      ...
      <div className="min-h-full bg-gray-50 p-6 md:p-10">
        <div className="flex justify-center items-center py-12">
          <div className="animate-pulse text-center">
            ...
            <div className="w-64 h-4 bg-gray-200 rounded mb-2 mx-auto"></div>
  ...
  return (
    <div className="min-h-full bg-gray-50 p-6 md:p-10 space-y-6">
      {/* Error Message */}
      {error && (
              ...
              <div className="relative">
                <div className="w-20 h-20 rounded-full bg-white/20 flex items-center justify-center overflow-hidden">
                  {userData?.profile_picture ? (
                    <img
                      src={resolveUploadUrl(userData.profile_picture)}
                      alt="Profile"
